...
TEST(ArcReplacerTest, DISABLED_SampleTest) {
  ...
  // We set frame 5 as non-evictable. These pages all go to mru list
  // We now have frames [][(1,f0), (2,f1), (3,f2), (4,f3), (5,f4), p(6,f5)]![][]
  arc_replacer.RecordAccess(0, 1);
  arc_replacer.RecordAccess(1, 2);
  arc_replacer.RecordAccess(2, 3);
  arc_replacer.RecordAccess(3, 4);
  arc_replacer.RecordAccess(4, 5);
  arc_replacer.RecordAccess(5, 6);
  ...
  arc_replacer.SetEvictable(5, false);
  ...
  // Record an access for frame 0. Now frame 0 goes into mfu list
  arc_replacer.RecordAccess(0, 1);
  // Now [][(2,f1), (3,f2), (4,f3), (5,f4), p(6,f5)]![(1,f0)][] p=0
  //
  ...
  // Now [(2,_), (3,_), (4,_)][(5,f4), p(6,f5)]![(1,f0)][] p=0

  // Insert new page 7 on frame 1, this should NOT be a hit on ghost
  // list since we've never seen page 7, this goes into mru list
  arc_replacer.RecordAccess(1, 7);
  arc_replacer.SetEvictable(1, true);
  // Insert page 2 on frame 2, this should be a hit on mru ghost list
  ...
  arc_replacer.SetEvictable(2, true);
  // Now [(3,_), (4,_)][(5,f4), p(6,f5), (7,f1)]![(2,f2), (1,f0)][] p=1
  ASSERT_EQ(4, arc_replacer.Size());

  // Continue to insert page 3 on frame 3 and page 4 on frame 6
  arc_replacer.RecordAccess(3, 3);
  arc_replacer.SetEvictable(3, true);
  arc_replacer.RecordAccess(6, 4);
  arc_replacer.SetEvictable(6, true);
  // Now [][(5,f4), p(6,f5), (7,f1)]![(4,f6), (3,f3), (2,f2), (1,f0)][] p=3
  ASSERT_EQ(6, arc_replacer.Size());

  // Evict an entry, now target size is 3, we still evict from mru
  ASSERT_EQ(4, arc_replacer.Evict());
  // Now [(5,_)][p(6,f5), (7,f1)]![(4,f6), (3,f3), (2,f2), (1,f0)][] p=3
  // Evict another entry, this time mru is smaller than target,
  // mfu is victimized
  ASSERT_EQ(0, arc_replacer.Evict());
  // Now [(5,_)][p(6,f5), (7,f1)]![(4,f6), (3,f3), (2,f2)][(1,_)] p=3

  // Make another access to page 1 on frame 4, now page 1 is back to mfu
  // with a different frame, also p is adjusted down by 1/1=1
  arc_replacer.RecordAccess(4, 1);
  arc_replacer.SetEvictable(4, true);
  // Now [(5,_)][p(6,f5), (7,f1)]![(1,f4), (4,f6), (3,f3), (2,f2)][] p=2
  ASSERT_EQ(5, arc_replacer.Size());

  // We evict again, this time target size is 2, we evict from mru,
  // note that page 6 is pinned. Victim is page 7
  // Now [(5,_), (7,_)][p(6,f5)]![(1,f4), (4,f6), (3,f3), (2,f2)][] p=2
  ASSERT_EQ(1, arc_replacer.Evict());
}
...
TEST(ArcReplacerTest, DISABLED_SampleTest2) {
  ...
  // Insert a new page 4 with frame 2. This is case 4A
  // and ghost pages 1 should be driven out
  arc_replacer.RecordAccess(2, 4);
  arc_replacer.SetEvictable(2, true);
  // Now [(2,_), (3,_)][(4,f2)]![][] p=0

  // Access page 1 on frame 1, it should NOT be a hit on
  // the ghost list. Ghost page 2 should be driven out
  arc_replacer.RecordAccess(1, 1);
  arc_replacer.SetEvictable(1, true);
  ASSERT_EQ(2, arc_replacer.Size());
  // Now [(3,_)][(4,f2), (1,f1)]![][] p=0

  // Access page 3 with frame 0, this should be a ghost hit,
  // page 3 is placed on mfu and target size is bumped up by 1
  arc_replacer.RecordAccess(0, 3);
  arc_replacer.SetEvictable(0, true);
  // Now [][(4,f2), (1,f1)]![(3,f0)][] p=1
  ...
  arc_replacer.RecordAccess(2, 2);
  arc_replacer.SetEvictable(2, true);
  // Now [][(6,f0),(7,f1),(2,f2)]![][(5,_),(4,_),(1,_)] p=3

  // Evict a page 6
  ASSERT_EQ(0, arc_replacer.Evict());
  // Now [(6,_)][(7,f1),(2,f2)]![][(5,_),(4,_),(1,_)] p=3
  // And access page 3 who was removed
  // then this is case 4A, ghost page 6 will be removed
  arc_replacer.RecordAccess(0, 3);
  arc_replacer.SetEvictable(0, true);
  // Now [][(7,f1),(2,f2),(3,f0)]![][(5,_),(4,_),(1,_)] p=3
